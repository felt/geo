defmodule Geo.MultiLineString do
  ...
  if Code.ensure_loaded?(Ecto.Type) do
    ...
    def cast(%{"type" => "MultiLineString", "coordinates" => _} = multi_line_string),
      do: {:ok, Geo.JSON.decode(multi_line_string)}

    if Code.ensure_loaded?(Poison) do
      def cast(multi_line_string) when is_binary(multi_line_string),
        do: {:ok, Poison.decode!(multi_line_string) |> Geo.JSON.decode()}
    end
